    ...
    while True:
        ...
        if byte == 0:
            ...
            if special_byte == 0:  # End of line.
                ...
                if (
                    y != -1
                ):  # y == -1 when the "special_byte == 0" end of line if-statement above was reached and y was 0.
                    for _ in range(width - x - 1):
                        x += 1
                        decompressed[y].append(0)
                ...
                special_byte == 2
            ):  # Delta. The 2 bytes following the escape contain unsigned values indicating the offset to the right and up of the next pixel from the current position.
                byte_index += 1
                ...
                for _ in range(
                    offset_y * width
                ):  # TODO: Does this work when offset_y > 1?
                    if x == width - 1:
                        x = -1
                        y -= 1
...
def get_pixel_array(img, width, height, palette):
    ...
    decoded_pixel_index_array = get_decoded_pixel_index_array(
        pixel_index_array_bytes, width, height
    )
