...
def playsound_warning(line, file_path, line_number):
    ...
    if (
        ...
        # Print a warning.
        warnings.mods_warnings.append(
            "'{}' line {}: {} -> {}".format(file_path, line_number, pattern, message)
        )


def regex_replace(all_lines):
    all_lines = replace_without_using_match(
        all_lines, "Framerate = (.*)", "SpriteAnimMode = 7"
    )
    all_lines = replace_without_using_match(all_lines, "\tPlayerCount = (.*)\n", "")
    all_lines = replace_without_using_match(all_lines, "\tTeamCount = (.*)\n", "")
    all_lines = replace_without_using_match(
        all_lines, "CollidesWithTerrainWhileAttached(?!.*=)", "CanCollideWithTerrain"
    )  # TODO: Should only be applied to Lua files.

    all_lines = replace_using_match(
        all_lines, "ModuleName = (.*) Tech\n", "ModuleName = {}\n\tIsFaction = 1\n"
    )
    all_lines = replace_using_match(all_lines, "FundsOfTeam(.*) =", "Team{}Funds =")

    all_lines = replace_using_match(
        all_lines, "[bB]ase\.rte(.*?)\.wav", "Base.rte{}.flac"
    )
    all_lines = replace_using_match(
        all_lines, "[bB]rowncoats\.rte(.*?)\.wav", "Browncoats.rte{}.flac"
    )
    all_lines = replace_using_match(
        all_lines, "[cC]oalition\.rte(.*?)\.wav", "Coalition.rte{}.flac"
    )
    all_lines = replace_using_match(
        all_lines, "[dD]ummy\.rte(.*?)\.wav", "Dummy.rte{}.flac"
    )
    all_lines = replace_using_match(
        all_lines, "[iI]mperatus\.rte(.*?)\.wav", "Imperatus.rte{}.flac"
    )
    all_lines = replace_using_match(
        all_lines, "[mM]uIlaak\.rte(.*?)\.wav", "MuIlaak.rte{}.flac"
    )
    all_lines = replace_using_match(
        all_lines, "[rR]onin\.rte(.*?)\.wav", "Ronin.rte{}.flac"
    )
    all_lines = replace_using_match(
        all_lines, "[tT]echion\.rte(.*?)\.wav", "Techion.rte{}.flac"
    )
    all_lines = replace_using_match(
        all_lines, "[uU]zira\.rte(.*?)\.wav", "Uzira.rte{}.flac"
    )

    all_lines = replace_using_match(
        all_lines, "MovableMan:RemoveActor\((.*?)\)", "{}.ToDelete = true"
    )

    all_lines = special_replace_using_matches(
        all_lines,
        regex_replace_particle,
        "ParticleNumberToAdd = (.*)"
        "\n\tAddParticles = (.*)"
        "\n\t\tCopyOf = (.*)"
        "\n",
    ...
    all_lines = special_replace_using_matches(
        all_lines,
        regex_replace_sound_priority,
        "SoundContainer(((?!SoundContainer).)*)Priority",
        "SoundContainer{}// Priority",
        dotall=True,
    )
    all_lines = special_replace_using_matches(
        all_lines,
        regex_replace_lua_max_mass,
        "(\w+)\.MaxMass[ =]",
        "{}.MaxInventoryMass + {}.Mass ",
        dotall=False,
    )
...
def special_replace_using_matches(all_lines, fn, pattern, replacement, dotall):
    ...
    if len(matches) > 0:
        ...
        return re.sub(
            pattern, replacement, escaped_all_lines, flags=re.DOTALL if dotall else 0
        ).format(*new)
    return all_lines
...
def regex_replace_particle(all_lines, pattern, replacement, matches):
    ...
    # 0, 1, 2 -> 1, 2, 0
    new[0::3], new[1::3], new[2::3] = new[1::3], new[2::3], new[0::3]
