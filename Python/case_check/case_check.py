...
from Python.case_check.case_check_errors import (
...
def init_glob(output_folder_path, input_path):
    ...
    _path_glob = [
        p.relative_to(output_folder_path).as_posix()[: -len(p.suffix)]
        + p.suffix.lower()
    ...
    _modules = [
        p.relative_to(output_folder_path).as_posix()
        for p in sorted(Path(output_folder_path).glob("*.rte"))
    ]
    _modules.extend(
        [
...
def case_check(all_lines, input_file_path, output_file_path):
    ...
    for line_number, line in enumerate(all_lines.split("\n"), start=1):
        ...
        if Path(input_file_path).suffix == ".ini":
            ...
            file_case_match.update(
                case_check_ini_line(line, output_file_path, line_number)
            )
        elif Path(input_file_path).suffix == ".lua":
            file_case_match.update(
                case_check_lua_line(line, output_file_path, line_number)
            )
...
def check_file_exists(path):
    ...
    if (path in _path_glob) or (
        path[-4:] in _image_ext
        and any(
            (path[:-4] == image or (path[:-4] + "000") == image) for image in _images
        )
    ):
        return ""
...
def case_check_ini_line(line, file_name, line_number):
    ...
    if any(
        line_uncommented.startswith(include_op) for include_op in _ini_file_includes
    ):
...
def lua_include_exists(included_file):
    ...
    """
    if included_file in _path_glob or any(
        included_file + ".lua" in file for file in _path_glob
    ):
        return ""
            ...
            for match in contents.groups():
                if ".rte" in match.partition("/")[0]:
                    ...
                    if module in _modules:
                        ...
                        return {
                            module: _modules[
                                [m.lower() for m in _modules].index(module.lower())
                            ]
                        }
                    else:
