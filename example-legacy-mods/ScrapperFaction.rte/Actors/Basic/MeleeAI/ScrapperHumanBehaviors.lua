...
ScrapperHumanBehaviors = {}

function ScrapperHumanBehaviors.GetTeamShootingSkill(team)
	local skill = 50
...
function ScrapperHumanBehaviors.SetShootingSkill()
	-- returns average skill for all active teams
	AI.aimSpeed, AI.aimSkill = ScrapperHumanBehaviors.GetTeamShootingSkill(-1)
end

function ScrapperHumanBehaviors.GetShootingSkill()
	-- returns average skill for all active teams
	return ScrapperHumanBehaviors.GetTeamShootingSkill(-1)
end

-- spot targets by casting a ray in a random direction
function ScrapperHumanBehaviors.LookForTargets(AI, Owner)
...
function ScrapperHumanBehaviors.CheckEnemyLOS(AI, Owner)
	if not AI.Enemies then	-- add all enemy actors on our screen to a table and check LOS to them, one per frame
		...
		return ScrapperHumanBehaviors.LookForTargets(AI, Owner)	-- cast rays like normal actors occasionally
	else
			...
			if MovableMan:ValidMO(Enemy) then
				...
				if Enemy.ClassName == "ADoor" then
					...
					if Door and Door:IsAttached() then
						...
						return ScrapperHumanBehaviors.LookForTargets(AI, Owner)		-- this door is destroyed, cast rays like normal actors
					end
				else
...
function ScrapperHumanBehaviors.CalculateThreatLevel(MO, Owner)
...
function ScrapperHumanBehaviors.ProcessAlarmEvent(AI, Owner)
	AI.AlarmPos = nil
...
function ScrapperHumanBehaviors.GetGrenadeAngle(AimPoint, TargetVel, StartPos, muzVel)
...
function ScrapperHumanBehaviors.EquipPreferredWeapon(AI, Owner)
...
function ScrapperHumanBehaviors.EquipPrimaryWeapon(AI, Owner)
...
function ScrapperHumanBehaviors.EquipSecondaryWeapon(AI, Owner)
	Owner:EquipDeviceInGroup("Secondary Weapons", true)
	return false
end

-- in sentry behavior the agent only looks for new enemies, it sometimes sharp aims to increase spotting range
function ScrapperHumanBehaviors.Sentry(AI, Owner, Abort)
	local sweepUp = true
...
function ScrapperHumanBehaviors.Patrol(AI, Owner, Abort)
...
function ScrapperHumanBehaviors.GoldDig(AI, Owner, Abort)
...
function ScrapperHumanBehaviors.BrainSearch(AI, Owner, Abort)
	if AI.PlayerPreferredHD then
		Owner:EquipNamedDevice(AI.PlayerPreferredHD, true)
	end
...
function ScrapperHumanBehaviors.WeaponSearch(AI, Owner, Abort)
	local minDist
	local Devices = {}
...
function ScrapperHumanBehaviors.ToolSearch(AI, Owner, Abort)
	local minDist
...
function ScrapperHumanBehaviors.GoToWpt(AI, Owner, Abort)
...
function ScrapperHumanBehaviors.GoProne(AI, Owner, TargetPos, targetID)
...
function ScrapperHumanBehaviors.GetProjectileData(Owner)
...
function ScrapperHumanBehaviors.ShootTarget(AI, Owner, Abort)
	...
	if not AI.flying and AI.Target.Vel.Largest < 4 and ScrapperHumanBehaviors.GoProne(AI, Owner, AimPoint, AI.Target.ID) then
	...
	while true do
		...
		if Owner.FirearmIsReady then
			...
			if not PrjDat or PrjDat.MagazineName ~= Weapon.Magazine.PresetName then
				PrjDat = ScrapperHumanBehaviors.GetProjectileData(Owner)
				...
				if checkAim then
					...
					elseif range < PrjDat.rng then
						...
						aimTarget = ScrapperHumanBehaviors.GetAngleToHit(PrjDat, Dist)
						if aimTarget then
							AI.canHitTarget = true
						else
							AI.canHitTarget = false
...
function ScrapperHumanBehaviors.ThrowTarget(AI, Owner, Abort)
			...
			if scan < 1 then
				...
				if ID > 0 and ID ~= rte.NoMOID then	-- MO found					 
					...
					if Owner.ThrowableIsReady then
						...
						if Grenade then
							aim = ScrapperHumanBehaviors.GetGrenadeAngle(AimPoint, Vector(), Grenade.MuzzlePos, Grenade.MaxThrowVel)
							if aim then
								ThrowTimer:Reset()
								...
								aim = ScrapperHumanBehaviors.GetGrenadeAngle(AimPoint, Vector(), Grenade.MuzzlePos, (Grenade.MaxThrowVel+Grenade.MinThrowVel)/2)
								if aim then
...
function ScrapperHumanBehaviors.AttackTarget(AI, Owner, Abort)
	...
	while true do
		...
		if Owner.EquippedItem then
			if Owner.EquippedItem:HasObjectInGroup("Diggers")	-- attack with digger
			or Owner.EquippedItem:HasObjectInGroup("Melee Weapons") then	-- or melee weapon (4zK)
...
function ScrapperHumanBehaviors.MoveAroundActor(AI, Owner, Abort)
...
function ScrapperHumanBehaviors.GetAngleToHit(PrjDat, Dist)
...
function ScrapperHumanBehaviors.ShootArea(AI, Owner, Abort)
	...
	if not AI.flying and AI.UnseenTarget.Vel.Largest < 12 and ScrapperHumanBehaviors.GoProne(AI, Owner, AI.UnseenTarget.Pos, AI.UnseenTarget.ID) then
		aimTime = aimTime + 500
	end
	...
	local aim
	local PrjDat = ScrapperHumanBehaviors.GetProjectileData(Owner)
	...
	if Dist.Magnitude < PrjDat.rng then
		aim = ScrapperHumanBehaviors.GetAngleToHit(PrjDat, Dist)
	else
...
function ScrapperHumanBehaviors.FaceAlarm(AI, Owner, Abort)
	if AI.AlarmPos then
...
function ScrapperHumanBehaviors.PinArea(AI, Owner, Abort)
...
local Proxy = {}
local Mt = {
	__index = ScrapperHumanBehaviors,
	__newindex = function(Table, k, v)
		error("The ScrapperHumanBehaviors table is read-only.", 2)
	end
}
setmetatable(Proxy, Mt)
ScrapperHumanBehaviors = Proxy
