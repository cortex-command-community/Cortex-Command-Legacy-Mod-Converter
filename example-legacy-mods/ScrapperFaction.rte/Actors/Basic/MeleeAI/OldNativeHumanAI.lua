...
dofile("ScrapperFaction.rte/Actors/Basic/MeleeAI/OldHumanBehaviors.lua")	--change this to point to the right location

MeleeNativeHumanAI = {}

function MeleeNativeHumanAI:Create(Owner)
	local Members = {}
	...
	if Owner.Jetpack then
		Members.jetImpulse = Owner.Jetpack:EstimateImpulse(false) * FrameMan.PPM / TimerMan.DeltaTimeSecs
		Members.jetBurstAcc = (Owner.Jetpack:EstimateImpulse(true) * FrameMan.PPM / TimerMan.DeltaTimeSecs - Members.jetImpulse) * TimerMan.DeltaTimeSecs * TimerMan.DeltaTimeSecs * 0.5
	...
	if Owner:HasObjectInGroup("Brains") or Owner:HasObjectInGroup("Snipers") then
		Members.SpotTargets = MeleeHumanBehaviors.CheckEnemyLOS
	else
		Members.SpotTargets = MeleeHumanBehaviors.LookForTargets
	end
	...
	if ActivityMan:GetActivity():IsPlayerTeam(Owner.Team) then
		...
		if Owner.Jetpack then
			Owner.Jetpack.Throttle = 0.2	-- increase jetpack strength slightly to compensate for AI ineptitude
		end
	end
	
	-- set shooting skill, do this after the isPlayerOwned check
	MeleeHumanBehaviors.SetShootingSkill(Members)
...
function MeleeNativeHumanAI:Update(Owner)
	...
	if FoundMO then
			...
			if FoundMO.Team == Owner.Team then	-- found an ally
				...
				if FoundMO then
					if self.Target then
						...
						if MeleeHumanBehaviors.CalculateThreatLevel(FoundMO, Owner) > 
							MeleeHumanBehaviors.CalculateThreatLevel(self.Target, Owner) + 0.5
						then
							self.OldTargetPos = Vector(self.Target.Pos.X, self.Target.Pos.Y)
							self.Target = FoundMO
		...
		if self.ReloadTimer:IsPastSimMS(8000) then	-- check if we need to reload
			if Owner.FirearmNeedsReload then
				self.ReloadTimer:Reset()
				Owner:ReloadFirearm()
			elseif not MeleeHumanBehaviors.EquipPreferredWeapon(self, Owner) then	-- make sure we equip a preferred or a primary weapon if we have one
				self.ReloadTimer:Reset()
			end
		end
	end
	...
	if self.squadShoot then
		...
		elseif not self.Target and not self.UnseenTarget then	
			if self.AlarmTimer:IsPastSimTimeLimit() and MeleeHumanBehaviors.ProcessAlarmEvent(self, Owner) then
...
function MeleeNativeHumanAI:CreateSentryBehavior(Owner)
	if not self.Target then
		...
		self.NextBehavior = coroutine.create(MeleeHumanBehaviors.Sentry)	-- replace "HumanBehaviors.Sentry" with the function name of your own sentry behavior
		self.NextCleanup = nil
		self.NextBehaviorName = "Sentry"
	end
end

function MeleeNativeHumanAI:CreatePatrolBehavior(Owner)
	self.NextBehavior = coroutine.create(MeleeHumanBehaviors.Patrol)
	self.NextCleanup = nil
	self.NextBehaviorName = "Patrol"
end

function MeleeNativeHumanAI:CreateGoldDigBehavior(Owner)
	...
	self.NextBehavior = coroutine.create(MeleeHumanBehaviors.GoldDig)
	self.NextCleanup = nil
	self.NextBehaviorName = "GoldDig"
end

function MeleeNativeHumanAI:CreateBrainSearchBehavior(Owner)
	self.NextBehavior = coroutine.create(MeleeHumanBehaviors.BrainSearch)
	self.NextCleanup = nil
	self.NextBehaviorName = "BrainSearch"
end

function MeleeNativeHumanAI:CreateGetToolBehavior(Owner)
	if Owner.AIMode ~= Actor.AIMODE_SQUAD then
		self.NextBehavior = coroutine.create(MeleeHumanBehaviors.ToolSearch)
		self.NextCleanup = nil
		self.NextBehaviorName = "ToolSearch"
	end
end

function MeleeNativeHumanAI:CreateGetWeaponBehavior(Owner)
	if Owner.AIMode ~= Actor.AIMODE_SQUAD then
		self.NextBehavior = coroutine.create(MeleeHumanBehaviors.WeaponSearch)
		self.NextCleanup = nil
		self.NextBehaviorName = "WeaponSearch"
	end
end

function MeleeNativeHumanAI:CreateGoToBehavior(Owner)
	self.NextGoTo = coroutine.create(MeleeHumanBehaviors.GoToWpt)
...
function MeleeNativeHumanAI:CreateAttackBehavior(Owner)
	self.ReloadTimer:Reset()
	self.TargetLostTimer:Reset()
	
	if Owner:EquipFirearm(true) then
		if Owner.EquippedItem:HasObjectInGroup("Melee Weapons") then
			self.NextBehavior = coroutine.create(MeleeHumanBehaviors.AttackTarget)
			self.NextBehaviorName = "AttackTarget"		
		else
			self.NextBehavior = coroutine.create(MeleeHumanBehaviors.ShootTarget)
			self.NextBehaviorName = "ShootTarget"
		end
	elseif Owner.AIMode ~= Actor.AIMODE_SQUAD and Owner:EquipThrowable(true) then
		self.NextBehavior = coroutine.create(MeleeHumanBehaviors.ThrowTarget)
...
function MeleeNativeHumanAI:CreateHtHBehavior(Owner)
...
function MeleeNativeHumanAI:CreateSuppressBehavior(Owner)
	if Owner:EquipFirearm(true) then
		self.NextBehavior = coroutine.create(MeleeHumanBehaviors.ShootArea)
...
function MeleeNativeHumanAI:CreateMoveAroundBehavior(Owner)
	self.NextGoTo = coroutine.create(MeleeHumanBehaviors.MoveAroundActor)
...
function MeleeNativeHumanAI:CreateFaceAlarmBehavior(Owner)
	self.NextBehavior = coroutine.create(MeleeHumanBehaviors.FaceAlarm)
	self.NextBehaviorName = "FaceAlarm"
	self.NextCleanup = nil
end

function MeleeNativeHumanAI:CreatePinBehavior(Owner)
	if self.OldTargetPos and Owner:EquipFirearm(true) then
		self.NextBehavior = coroutine.create(MeleeHumanBehaviors.PinArea)
