	...
	if self:GetNumberValue("Block Mode") == 1 then
		...
		if actor and IsAHuman(actor) then
			...
			if self:GetNumberValue("Actively Blocking") == 1 then
				...
				-- self.Scale = 0; 	-- doesn't collide with enemies
				if self:GetNumberValue("Randomization") == 0 then
					fake = CreateMOSRotating("ScrapperFaction.rte/Club A Fake");
				elseif self:GetNumberValue("Randomization") == 1 then
					fake = CreateMOSRotating("ScrapperFaction.rte/Club B Fake");
				elseif self:GetNumberValue("Randomization") == 2 then
					fake = CreateMOSRotating("ScrapperFaction.rte/Club C Fake");
				elseif self:GetNumberValue("Randomization") == 3 then
					fake = CreateMOSRotating("ScrapperFaction.rte/Club D Fake");
				elseif self:GetNumberValue("Randomization") == 4 then
					fake = CreateMOSRotating("ScrapperFaction.rte/Club E Fake");
				end
				fake.Pos = self.Pos; 
				fake.Vel = self.Vel;
	...
	if self:GetNumberValue("Block Mode") == 0 then
		...
		if actor and IsAHuman(actor) then
			...
			if self:IsActivated() and self:GetNumberValue("Striking") == 0 and self:GetNumberValue("Recovery") == 0 and self.recoveryTimer:IsPastSimMS(200) then
				...
				sfx = CreateAEmitter("Club Backswing");
				sfx.Pos = self.Pos;
				MovableMan:AddParticle(sfx);
			...
			if self:GetNumberValue("Striking") == 1 then
				...
				if parent then			
					parent:SetNumberValue("Meleed", 1);					
				end
				...
				--fake.HFlipped = self.HFlipped
				if self.HFlipped then
					fake.RotAngle = rotang;	-- HFlipped doesn't work
				else
					fake.RotAngle = rotang;
				end
				fake.AngularVel = self.AngularVel;
				...
				if self.ray > 0 then
					...
					if self.effectSpawn == true then
						...
						for i = 1, math.random(1, 2) do
							local Effect = CreateMOPixel("Particle Club", "ScrapperFaction.rte")
							if Effect then
								Effect.Vel = self.Vel + (Vector(70, 0):GetXFlipped(self.HFlipped):RadRotate(self.RotAngle));
								Effect.Pos = self.Pos + (Vector(0, -5):RadRotate(self.RotAngle));
