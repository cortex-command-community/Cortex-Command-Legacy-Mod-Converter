		...
		if self:PlayerActive(player) and self:PlayerHuman(player) then
			if not self:GetPlayerBrain(player) then
				...
				if not foundBrain then
					...
					foundBrain = RandomAHuman("Heavy Infantry", CSRT tech)
					local item = RandomHDFirearm("Light Weapons", CSRT tech)
					if item then
						foundBrain:AddInventoryItem(item)
					end
					local item = RandomHDFirearm("Secondary Weapons", CSRT tech)
					if item then
						foundBrain:AddInventoryItem(item)
					end
					foundBrain:AddInventoryItem(CreateHDFirearm("Ronin/Shovol"))
