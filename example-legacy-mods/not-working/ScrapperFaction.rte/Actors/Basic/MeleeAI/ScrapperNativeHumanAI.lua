...
dofile("ScrapperFaction.rte/Actors/Basic/MeleeAI/ScrapperHumanBehaviors.lua")

ScrapperNativeHumanAI = {}

function ScrapperNativeHumanAI:Create(Owner)
	local Members = {}
	...
	if Owner:HasObjectInGroup("Brains") or Owner:HasObjectInGroup("Snipers") then
		Members.SpotTargets = ScrapperHumanBehaviors.CheckEnemyLOS
	else
		Members.SpotTargets = ScrapperHumanBehaviors.LookForTargets
	end
	...
	-- set shooting skill
	Members.aimSpeed, Members.aimSkill = ScrapperHumanBehaviors.GetTeamShootingSkill(Owner.Team)
...
function ScrapperNativeHumanAI:Update(Owner)
	...
	if FoundMO then
			...
			if FoundMO.Team == Owner.Team then	-- found an ally
				...
				if FoundMO then
					if self.Target then
						...
						if ScrapperHumanBehaviors.CalculateThreatLevel(FoundMO, Owner) > 
							ScrapperHumanBehaviors.CalculateThreatLevel(self.Target, Owner) + 0.5
						then
							self.OldTargetPos = Vector(self.Target.Pos.X, self.Target.Pos.Y)
							self.Target = FoundMO
		...
		if self.ReloadTimer:IsPastSimMS(8000) then	-- check if we need to reload
			if Owner.FirearmNeedsReload then
				self.ReloadTimer:Reset()
				Owner:ReloadFirearm()
			elseif not ScrapperHumanBehaviors.EquipPreferredWeapon(self, Owner) then	-- make sure we equip a preferred or a primary weapon if we have one
				self.ReloadTimer:Reset()
			end
		end
	end
	
	--self.squadShoot = false	-- let's not update this every frame (4zK)
			...
			if Owner.AIMode == Actor.AIMODE_SQUAD then
				...
				if not self.jump and self.lateralMoveState == Actor.LAT_STILL then
					...
					if Leader and Leader.EquippedItem and SceneMan:ShortestDistance(Owner.Pos, Leader.Pos, false).Largest < (Leader.Height + Owner.Height) * 0.5 then
							...
							if LeaderWeapon:IsWeapon() then
								...
								if Owner.FirearmIsReady then
									...
									if IsHDFirearm(Owner.EquippedItem) and Leader:GetController():IsState(Controller.WEAPON_FIRE) then
										...
										then
											-- trying to slightly displace full auto shots to create a more dense fire rate (4zK)

											if OwnerWeapon.FullAuto == true then-- and self.squadShoot == false
						...
						-- squad grenade throw mechanic (4zK)
						elseif IsTDExplosive(Leader.EquippedItem) and Leader:IsPlayerControlled() == true then

							local LeaderWeapon = ToTDExplosive(Leader.EquippedItem);
							if LeaderWeapon:HasObjectInGroup("Grenades") and Owner:HasObjectInGroup("Grenades") then	-- both have to be "grenades" not "bombs"
								...
								if Leader:GetController():IsState(Controller.WEAPON_FIRE) then
									...
									Owner:EquipDeviceInGroup("Grenades", true);
					...
					if Leader and Leader.AIMode == Actor.AIMODE_GOLDDIG then	-- make entire squad dig gold (4zK)
	...
	if self.squadShoot then
		...
		if Owner.FirearmIsSemiAuto and self.SquadShootTimer:IsPastSimMS(Owner.FirearmActivationDelay+self.SquadShootDelay) then
			...
			self.squadShoot = false
			self.squadShootDelay = math.random(50,100);	-- 4zK
		end
	else
		...
		elseif not self.Target and not self.UnseenTarget then	-- no targets
			-- medikit script by 4zK --
			if not Owner:IsPlayerControlled()		-- if a cpu
			and Owner.Health < (Owner.MaxHealth/2)		-- at half health
			and Owner:HasObject("Medikit") then		-- has a medikit

				Owner:EquipNamedDevice("Medikit", true);-- equip it
				self.usemedikit = true;			-- and "fire" to activate the item

			elseif self.usemedikit == true then		-- we just healed with a medikit
				self.usemedikit = false;		-- revert
				Owner:EquipFirearm(true);		-- try to equip a weapon
			-- carry on with script --
			elseif self.AlarmTimer:IsPastSimTimeLimit() and ScrapperHumanBehaviors.ProcessAlarmEvent(self, Owner) then
	...
	if self.squadShoot then
		...
		self.Ctrl:SetState(Controller.WEAPON_FIRE, (self.fire or self.usemedikit))	-- medikits
	end
...
function ScrapperNativeHumanAI:Destroy(Owner)
	-- Tell the coroutines to abort to avoid memory leaks
	if self.GoToBehavior then
		local msg, done = coroutine.resume(self.GoToBehavior, self, Owner, true)
	end
...
-- functions that create behaviors. the default behaviors are stored in the ScrapperHumanBehaviors table. store your custom behaviors in a table to avoid name conflicts between mods.
function ScrapperNativeHumanAI:CreateSentryBehavior(Owner)
	if self.Target then
		self:CreateAttackBehavior(Owner)
	else
		...
		self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.Sentry)	-- replace "ScrapperHumanBehaviors.Sentry" with the function name of your own sentry behavior
		self.NextCleanup = nil
		self.NextBehaviorName = "Sentry"
	end
end

function ScrapperNativeHumanAI:CreatePatrolBehavior(Owner)
	self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.Patrol)
	self.NextCleanup = nil
	self.NextBehaviorName = "Patrol"
end

function ScrapperNativeHumanAI:CreateGoldDigBehavior(Owner)
	...
	self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.GoldDig)
	self.NextCleanup = nil
	self.NextBehaviorName = "GoldDig"
end

function ScrapperNativeHumanAI:CreateBrainSearchBehavior(Owner)
	self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.BrainSearch)
	self.NextCleanup = nil
	self.NextBehaviorName = "BrainSearch"
end

function ScrapperNativeHumanAI:CreateGetToolBehavior(Owner)
	if Owner.AIMode ~= Actor.AIMODE_SQUAD then
		self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.ToolSearch)
		self.NextCleanup = nil
		self.NextBehaviorName = "ToolSearch"
	end
end

function ScrapperNativeHumanAI:CreateGetWeaponBehavior(Owner)
	if Owner.AIMode ~= Actor.AIMODE_SQUAD then
		self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.WeaponSearch)
		self.NextCleanup = nil
		self.NextBehaviorName = "WeaponSearch"
	end
end

function ScrapperNativeHumanAI:CreateGoToBehavior(Owner)
	self.NextGoTo = coroutine.create(ScrapperHumanBehaviors.GoToWpt)
...
function ScrapperNativeHumanAI:CreateAttackBehavior(Owner)
	self.ReloadTimer:Reset()
	self.TargetLostTimer:Reset()

	if Owner:EquipFirearm(true) then
		if Owner.EquippedItem:HasObjectInGroup("Melee Weapons") then   
			self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.AttackTarget)
			self.NextBehaviorName = "AttackTarget"
		else
			self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.ShootTarget)
			self.NextBehaviorName = "ShootTarget"
		end
	elseif Owner.AIMode ~= Actor.AIMODE_SQUAD and Owner:EquipThrowable(true) then
		self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.ThrowTarget)
...
function ScrapperNativeHumanAI:CreateHtHBehavior(Owner)					-- has to be digger, not just "tool"
	if Owner.AIMode ~= Actor.AIMODE_SQUAD and self.Target and Owner:HasObjectInGroup("Diggers") then	--Owner:EquipDiggingTool(true) then
		self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.AttackTarget)
		self.NextBehaviorName = "AttackTarget"
		self.NextCleanup = function(AI)
			AI.fire = false
...
function ScrapperNativeHumanAI:CreateSuppressBehavior(Owner)
	if Owner:EquipFirearm(true) then
		self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.ShootArea)
...
function ScrapperNativeHumanAI:CreateMoveAroundBehavior(Owner)
	self.NextGoTo = coroutine.create(ScrapperHumanBehaviors.MoveAroundActor)
...
function ScrapperNativeHumanAI:CreateFaceAlarmBehavior(Owner)
	self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.FaceAlarm)
	self.NextBehaviorName = "FaceAlarm"
	self.NextCleanup = nil
end

function ScrapperNativeHumanAI:CreatePinBehavior(Owner)
	if self.OldTargetPos and Owner:EquipFirearm(true) then
		self.NextBehavior = coroutine.create(ScrapperHumanBehaviors.PinArea)
