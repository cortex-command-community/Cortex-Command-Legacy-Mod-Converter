...
dofile("ScrapperFaction.rte/Actors/Basic/MeleeAI/HumanBehaviors.lua")
...
function NativeHumanAI:Update(Owner)
	...
	if self.squadShoot then
		...
		elseif not self.Target and not self.UnseenTarget then	-- no targets
			...
			elseif self.AlarmTimer:IsPastSimTimeLimit() and HumanBehaviors.ProcessAlarmEvent(self, Owner) then
...
function NativeHumanAI:CreateAttackBehavior(Owner)
	self.ReloadTimer:Reset()
	self.TargetLostTimer:Reset()
	--[[ medikit mechanics (4zK)
	if Owner.Health < Owner.MaxHealth/2 and Owner:HasObject("Medikit") then
		self:EquipNamedDevice("Medikit", true);
