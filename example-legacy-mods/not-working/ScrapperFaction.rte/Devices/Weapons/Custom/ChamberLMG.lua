...
function Update(self)
	...
	elseif self.Magazine and self:IsActivated() and self.chamber == false then      
		if self.triggerPulled == false then        
			...
			sfx = CreateAEmitter("Pre Custom LMG");
			sfx.Pos = self.Pos;
	...
	if self.FiredFrame then	
		...
		if self.sfxSetMech ~= false then
			...
			sfxSetShot = CreateAEmitter("HiFi Custom LMG");
		...
		if self.sfxSetMech ~= false then
			...
			sfxSetShot = CreateAEmitter("Shell Custom LMG");
			sfxSetShot.Pos = self.Pos;
			MovableMan:AddParticle(sfxSetShot);
		...
		if self.sfxSetMech ~= false then
			...
			sfxSetShot = CreateAEmitter("CoreBass Custom LMG");
			sfxSetShot.Pos = self.Pos;
			MovableMan:AddParticle(sfxSetShot);
		...
		if self.ray < 0 or self.rayRight < 0 or self.rayLeft < 0 then	
			...
			if self.parent then
				...
				if self.parent:IsPlayerControlled() then
					...
					if self.sfxSetTail ~= false then
						...
						self.sfx = CreateAEmitter("Reflection Custom LMG");
					...
					if self.sfxSetTail ~= false then
						...
						self.sfx = CreateAEmitter("Reflection Weak Custom LMG");
						self.sfx.Pos = self.Pos;
						self.echoSound = ToAEmitter(self.sfx)
						MovableMan:AddParticle(self.sfx);
						self.echoMakingSound = true;
			...
			if self.sfxSetMech ~= false then
				...
				sfxSetShot = CreateAEmitter("Add Custom LMG");
				sfxSetShot.Pos = self.Pos;
				MovableMan:AddParticle(sfxSetShot);
			...
			if self.sfxSetShot ~= false then
				...
				sfxSetShot = CreateAEmitter("Noise Custom LMG");
				sfxSetShot.Pos = self.Pos;
				MovableMan:AddParticle(sfxSetShot);
			...
			if self.sfxSetMech ~= false then
				...
				sfxSetShot = CreateAEmitter("AddShort Custom LMG");
				sfxSetShot.Pos = self.Pos;
				MovableMan:AddParticle(sfxSetShot);
				...
				if self.ray > 200 or self.rayRight > 200 or self.rayLeft > 200 then
					...
					if self.parent:IsPlayerControlled() then
						...
						if self.sfxSetTail ~= false then
							...
							self.sfx = CreateAEmitter("Reflection Big Custom LMG Indoors");
							self.sfx.Pos = self.Pos;
							self.echoSound = ToAEmitter(self.sfx)
							MovableMan:AddParticle(self.sfx);
							self.echoMakingSound = true;
						...
						if self.sfxSetTail ~= false then
							...
							self.sfx = CreateAEmitter("Reflection Big Weak Custom LMG Indoors");
							self.sfx.Pos = self.Pos;
							self.echoSound = ToAEmitter(self.sfx)
							MovableMan:AddParticle(self.sfx);
							self.echoMakingSound = true;
					...
					if self.parent:IsPlayerControlled() then
						...
						if self.sfxSetTail ~= false then
							...
							self.sfx = CreateAEmitter("Reflection Custom LMG Indoors");
							self.sfx.Pos = self.Pos;
							self.echoSound = ToAEmitter(self.sfx)
							MovableMan:AddParticle(self.sfx);
							self.echoMakingSound = true;
						...
						if self.sfxSetTail ~= false then
							...
							self.sfx = CreateAEmitter("Reflection Weak Custom LMG Indoors");
							self.sfx.Pos = self.Pos;
							self.echoSound = ToAEmitter(self.sfx)
							MovableMan:AddParticle(self.sfx);
							self.echoMakingSound = true;
