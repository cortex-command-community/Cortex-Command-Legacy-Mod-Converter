...
function Update(self)   
	...
	if self:IsAttached() and MovableMan:IsActor(Unit) and Unit:IsInGroup("Z Steel Soldiers") and Unit.PresetName == "Pyro" then
		self.SharpLength = self.origSharpLength*1.2;
		self.ShakeRange = self.origShakeRange*0.8;
		self.SharpShakeRange = self.origSharpShakeRange*0.8;
		self.ReloadTime = self.origReloadTime*0.6;
