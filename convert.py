...
def regex_replace(all_lines):
	all_lines = regex_replace_particle(all_lines)
	all_lines = regex_replace_sound_priority(all_lines)
	all_lines = regex_replace_playsound(all_lines)
	return all_lines


def regex_replace_particle(all_lines):
	pattern = "ParticleNumberToAdd = (.*)\n\tAddParticles = MO(.*)\n\t\tCopyOf = (.*)\n"
	formatted_str = "AddGib = Gib\n\t\tGibParticle = MO{}\n\t\t\tCopyOf = {}\n\t\tCount = {}\n"
	...
	if len(matches) > 0:
		...
		all_lines = re.sub(pattern, formatted_str, all_lines).format(*new)
	
	return all_lines


def regex_replace_sound_priority(all_lines):
	# TODO: This pattern returns two items in each tuple, while we only need the first.
	# https://stackoverflow.com/a/406408/13279557
	# https://regex101.com/r/NdKaWs/2

	pattern = " Sound(((?! Sound).)*)Priority"
	formatted_str = " Sound{}// Priority"
